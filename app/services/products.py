...
async def create_product(
    session: AsyncSession, payload: ProductCreateRequest
) -> Product:
    """Create a product."""
...
async def update_product(
    session: AsyncSession, product_id: int, payload: ProductUpdateRequest
) -> Product:
...
async def get_products(
    ...
    # Build base query
    stmt = select(Product).where(Product.is_active, Product.stock > 0)
